...
function createMetaTree(
  ...
  const authenticate = token => {
    ...
    // The worker reports the auth change its request caused before resolving the request, since
    // Firebase notifies its listeners before settling the call and the worker preserves that order.
    const reported = reportSignIn ?
      Promise.resolve(handleAuthChange(user)).catch(_.noop) : Promise.resolve();
    return reported.then(() => user);
  };
  const bridge = {
...
test('authenticate rejects when its certification fails', async () => {
  const {deliver, dispatcher, metaTree} =
    createMetaTree({reportSignIn: true, authResults: {token: {uid: 'github:1'}}});
  dispatcher.intercept('certify', {
    onBefore: op => {
      if (!op.operand) return;
  ...
  await assert.rejects(metaTree.authenticate('token'), {message: 'cannot certify'});
...
test('authenticate rejects with AUTH_REJECTED when an interceptor refuses its user', async () => {
  ...
  const error = await settle(metaTree.authenticate('token'));
  assert.equal(error.code, 'AUTH_REJECTED');
  assert.equal(metaTree.root.user, null);
});
...
test('a second auth call waits for the first to finish', async () => {
  const {deliver, dispatcher, metaTree} = createMetaTree({
    reportSignIn: true,
    authResults: {'token-a': {uid: 'github:a'}, 'token-b': {uid: 'github:b'}}
  });
  await deliver(null);
  const log = [];
  dispatcher.intercept('certify', {
...
test('each auth call sees only its own certification failure', async () => {
  ...
  await assert.rejects(metaTree.authenticate('token-a'), {message: 'A rejected'});
...
test('an authentication that fails before certifying leaves the queue usable', async () => {
  const {deliver, dispatcher, metaTree} =
    createMetaTree({reportSignIn: true, authResults: {good: {uid: 'github:1'}}});
  await deliver(null);
  let failNext = true;
  ...
  assert.equal(
    await race(metaTree.authenticate('good')), 'resolved', 'the failed call stranded the queue');
...
test('a rejection survives the cleanup sign-out it triggers', async () => {
  const {dispatcher, metaTree} = createMetaTree({
    reportSignIn: true, reportSignOut: true, authResults: {token: {uid: 'github:1'}}
  });
  await deliverInitialSignOut(metaTree);
  dispatcher.intercept('certify', {onBefore: op => op.operand ? false : undefined});

  const error = await race(metaTree.authenticate('token'));
...
// Codex:  wrapping the pre-emptive null certification in the collection window opened it before the
// bridge request went out, so an unrelated callback delivered while that certification waited on
// its own interceptors failed the logout even though the worker sign-out itself succeeded.
test('a background failure before the logout request is not the logout\'s own', async () => {
  const {deliver, dispatcher, metaTree, unauthCalls} = createMetaTree();
  await deliver({uid: 'someone'});
  let releaseNull;
  const failures = [];
  dispatcher.intercept('certify', {
    onBefore: op => {
      if (!op.operand) return new Promise(resolve => {releaseNull = resolve;});
      if (op.operand.uid === 'background') return Promise.reject(new Error('background failed'));
    }
  });
  dispatcher.intercept('unauthenticate', {onFailure: op => {failures.push(op.error.message);}});

  const unauthenticated = race(metaTree.unauthenticate());
  await drain();
  assert.deepEqual(unauthCalls, [], 'sent the sign-out before its certification finished');

  // An unrelated candidate arrives and fails to certify, before the sign-out request exists.
  const background = Promise.resolve(deliver({uid: 'background'})).catch(() => undefined);
  await drain();
  releaseNull();
  await background;

  assert.equal(await unauthenticated, 'resolved', 'a background failure failed the logout');
  assert.equal(unauthCalls.length, 1, 'skipped the worker sign-out');
  // onFailure callbacks are dispatched on a timeout.
  await new Promise(resolve => {setTimeout(resolve, 0);});
  assert.deepEqual(failures, [], 'reported a background failure as a logout failure');
});

// Codex:  `Dispatcher.begin` rejects its `Promise.all` as soon as one handler fails, discarding the
// results, so a sibling handler's `false` verdict was lost and the rejected candidate was never
// signed out.  Firebase then kept a session the app had explicitly refused, with nothing to detect
// it.  The hook error is still what the caller sees.
test('a false verdict survives a sibling certify hook failing', async () => {
  const {deliver, dispatcher, metaTree, unauthCalls} = createMetaTree({
    reportSignIn: true, authResults: {token: {uid: 'refused'}}
  });
  await deliver(null);
  dispatcher.intercept('certify', {onBefore: op => op.operand ? false : undefined});
  dispatcher.intercept('certify', {
    onBefore: op => op.operand ? Promise.reject(new Error('sibling hook failed')) : undefined
  });

  const error = await race(metaTree.authenticate('token'));

  assert.equal(error.message, 'sibling hook failed', 'lost the hook error');
  assert.equal(metaTree.root.user, null, 'published a refused candidate');
  assert.equal(unauthCalls.length, 1, 'left the worker signed in as a refused user');
});

// Codex:  recording the verdicts must not cost the fail-fast rejection that `Promise.all` gave us.
// Waiting for every handler to settle hangs any operation whose sibling interceptor never settles,
// which is a hazard for reads and writes as much as for certifications.
test('an onBefore rejection is reported without waiting for its siblings', async () => {
  const {dispatcher} = createMetaTree();
  dispatcher.intercept('certify', {onBefore: () => Promise.reject(new Error('refused'))});
  // A sibling that never settles:  the rejection above still has to come through.
  dispatcher.intercept('certify', {onBefore: () => new Promise(() => undefined)});

  const error = await race(
    dispatcher.execute('auth', 'certify', null, {uid: 'github:1'}, () => undefined));

  assert.equal(error.message, 'refused', 'waited for a sibling that never settles');
});

// Codex:  a `false` verdict can arrive from a handler slower than the one that failed the
// certification.  The operation's result is already decided by then, but the refusal still has to
// clear the worker, or Firebase keeps a user the app refused.  That sign-out happens out of band,
// since waiting for it would hang the call on a handler that may never settle.
test('a refusal that arrives after a sibling failure still signs the worker out', async () => {
  const {deliver, dispatcher, metaTree, unauthCalls} = createMetaTree();
  await deliver(null);
  dispatcher.intercept('certify', {
    onBefore: op => op.operand ? Promise.reject(new Error('fast hook failed')) : undefined
  });
  dispatcher.intercept('certify', {
    // Refuses only after the sibling above has already failed the operation.
    onBefore: op =>
      op.operand ? new Promise(resolve => {setTimeout(() => resolve(false), 20);}) : undefined
  });

  const error = await settle(deliver({uid: 'refused'}));

  assert.equal(error.message, 'fast hook failed');
  assert.equal(metaTree.root.user, null, 'published a refused candidate');
  await drain();
...
// Codex:  the out-of-band cleanup above must not outlive its candidate.  A certification that fails
// fast leaves the queue immediately, so a newer user can be published before a slow refusal
// arrives;  signing out then would clear a session nobody refused.
test('a refusal overtaken by a newer user does not sign it out', async () => {
  const {deliver, dispatcher, metaTree, unauthCalls} = createMetaTree();
  await deliver(null);
  dispatcher.intercept('certify', {
    onBefore: op => op.operand && op.operand.uid === 'old' ?
      Promise.reject(new Error('fast hook failed')) : undefined
  });
  dispatcher.intercept('certify', {
    // Refuses the old candidate, but only well after it has already failed.
    onBefore: op => op.operand && op.operand.uid === 'old' ?
      new Promise(resolve => {setTimeout(() => resolve(false), 30);}) : undefined
  });

  const refused = settle(deliver({uid: 'old'}));
  await drain();
  // A newer user is certified and published while that refusal is still outstanding.
  await Promise.resolve(deliver({uid: 'newer'})).catch(() => undefined);
  assert.equal(metaTree.root.userid, 'newer');

  await refused;
  await new Promise(resolve => {setTimeout(resolve, 50);});

  assert.equal(metaTree.root.userid, 'newer', 'signed out a user nobody refused');
  assert.deepEqual(unauthCalls, [], 'cleaned up against a candidate that had been overtaken');
});
...
test('a background certification failure does not leak into the next call', async () => {
  ...
  await deliver(null);
  let release, paused;
  // Resolves once the background handler has actually been entered, so the test doesn't depend on
  // how many microtasks the dispatcher takes to get there.
...
test('a certification failure reaches the authenticate operation hooks', async () => {
  const {deliver, dispatcher, metaTree} =
    createMetaTree({reportSignIn: true, authResults: {token: {uid: 'u'}}});
  await deliver(null);
  const log = [];
  dispatcher.intercept('certify', {
    onBefore: op => op.operand ? Promise.reject(new Error('cannot certify')) : undefined
...
// pkaminski:  a rejected candidate's failure has to survive the cleanup notification whatever it
// is, not only when it carries the AUTH_REJECTED code.  Here the candidate's own onAfter throws and
// the sign-out's real null callback used to clear that error, so the call resolved successfully
// on a candidate its own interceptor had refused.
test('a hook failure survives the cleanup notification of the rejection it caused', async () => {
  const {dispatcher, metaTree} = createMetaTree({
    reportSignIn: true, reportSignOut: true, authResults: {token: {uid: 'candidate'}}
  });
  await deliverInitialSignOut(metaTree);
  dispatcher.intercept('certify', {
    onBefore: op => op.operand ? false : undefined,
  ...
  assert.equal(error.message, 'after hook failed', 'the cleanup notification buried the failure');
  assert.equal(metaTree.root.user, null, 'published a rejected candidate');
});

// pkaminski:  a callback delivered before the call issued its request cannot be its answer, which
// is a different boundary from the RPC's response.  A background candidate rejected while the call
// is still draining used to be reported as that call's own outcome.
test('a background rejection while a call drains is not the call\'s own', async () => {
  const {deliver, dispatcher, metaTree} = createMetaTree({
    reportSignOut: true, authResults: {token: {uid: 'mine'}}
  });
  await deliver(null);
  let releasePaused;
  dispatcher.intercept('certify', {
    onBefore: op => {
      if (!op.operand) return;
      if (op.operand.uid === 'paused') return new Promise(resolve => {releasePaused = resolve;});
      // Rejecting this candidate signs it out internally and raises AUTH_REJECTED.
      if (op.operand.uid === 'rejected') return false;
    }
  });

  // A background certification parks, keeping the chain busy.
  const paused = Promise.resolve(deliver({uid: 'paused'})).catch(() => undefined);
  await drain();

  // The call is issued but can't start yet, and another background candidate is refused meanwhile.
  const authenticated = race(metaTree.authenticate('token'));
  await Promise.resolve();
  const rejected = Promise.resolve(deliver({uid: 'rejected'})).catch(() => undefined);
  await drain();

  releasePaused();
  await Promise.all([paused, rejected]);
  Promise.resolve(deliver({uid: 'mine'})).catch(() => undefined);

  assert.equal(await authenticated, 'resolved', 'reported a background rejection as its own');
  assert.equal(metaTree.root.userid, 'mine');
});

// pkaminski:  the call must finish on the certification result as it stood when its RPC settled.
// The failing callback arrives while the call's *own* certification is still pending, so the window
// can't have closed yet;  that's the interval a drain-until-quiet misses.
test('a background failure after settlement does not fail the call', async () => {
  const {deliver, dispatcher, metaTree} = createMetaTree({authResults: {token: {uid: 'mine'}}});
  await deliver(null);
  let releaseMine;
  dispatcher.intercept('certify', {
    onBefore: op => {
      if (!op.operand) return;
      if (op.operand.uid === 'mine') return new Promise(resolve => {releaseMine = resolve;});
      if (op.operand.uid === 'late') return Promise.reject(new Error('late background failure'));
    }
  });

  const authenticated = race(metaTree.authenticate('token'));
  await Promise.resolve();
  // The call's own answer arrives and parks in its interceptor;  its RPC settles meanwhile.
  const mine = Promise.resolve(deliver({uid: 'mine'})).catch(() => undefined);
  await drain();
  // A later background candidate fails while this call's own certification is still pending.
  const late = Promise.resolve(deliver({uid: 'late'})).catch(() => undefined);
  await drain();

  releaseMine();
  await Promise.all([mine, late]);
