...
test('an authentication that fails before certifying frees its claim', async () => {
  const {deliver, dispatcher, metaTree} = createMetaTree();
  await deliver(null);
  let failNext = true;
  ...
  // This one dies without ever producing an auth callback, so it must not leave a claim behind.
  ...
  const outcome = await Promise.race([
    authenticated.then(_.constant('settled')),
    new Promise(resolve => {setTimeout(() => resolve('hung'), 250);})
  ]);
  assert.equal(outcome, 'settled', 'the abandoned claim swallowed this certification');
